...
import { Level, Difficulty, Dialogue } from '@/lib/database';
...
interface DialoguesClientProps {
  initialDialogues: Dialogue[];
  initialLevels: Level[];
  initialDifficulties: Difficulty[];
}

export default function DialoguesClient({ initialDialogues, initialLevels, initialDifficulties }: DialoguesClientProps) {
  const { userData } = useAuth();
  const [dialogues] = useState<Dialogue[]>(initialDialogues);
  const [levels] = useState<Level[]>(initialLevels);
  const [difficulties] = useState<Difficulty[]>(initialDifficulties.filter(d => d.isActive));
