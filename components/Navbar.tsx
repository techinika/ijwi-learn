...
import { GraduationCap, Video, FileText, Award, MessageCircle, LogOut, User, Menu, X, Trophy, Globe } from 'lucide-react';
import { useState, useEffect } from 'react';
import { dbService, Language } from '@/lib/database';

export default function Navbar() {
  const { user, userData, logout, signInWithGoogle, updateUserData } = useAuth();
  ...
  const currentLang = languages.find(l => l.code === userData?.preferredLanguage) || languages.find(l => l.code === 'en');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
        <div className="flex items-center justify-between">
          ...
          {user && userData && (
            <div className="hidden md:flex items-center gap-2">
              <NavLink href="/" icon={<GraduationCap size={18} />} active={isActive('/')}>Levels</NavLink>
              <NavLink href="/videos" icon={<Video size={18} />} active={isActive('/videos')}>Videos</NavLink>
              <NavLink href="/tests" icon={<FileText size={18} />} active={isActive('/tests')}>Tests</NavLink>
              <NavLink href="/certificates" icon={<Award size={18} />} active={isActive('/certificates')}>Certificates</NavLink>
              <NavLink href="/leaderboard" icon={<Trophy size={18} />} active={isActive('/leaderboard')}>Leaderboard</NavLink>
            </div>
          )}
              ...
              <>
                ...
                <div className="relative group">
                  <button className="flex items-center gap-2 p-1 rounded-full hover:bg-gray-100 transition">
                    {user.photoURL ? (
                      <img src={user.photoURL} alt="" className="w-9 h-9 rounded-full ring-2 ring-primary-200" />
                    ) : (
                  ...
                  <div className="absolute right-0 top-full mt-3 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
                    <div className="p-4 border-b border-gray-100">
                      ...
                      <div className="mt-2 flex gap-3 text-xs">
                        <span className="bg-primary-100 text-primary-700 px-2 py-1 rounded-full">{userData?.totalPoints || 0} pts</span>
                        <span className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full">{userData?.consecutivePasses || 0} streak</span>
                      </div>
                    </div>
                    <div className="border-t border-gray-100">
                      <div className="relative">
                        ...
                        >
                          <Globe size={16} />
                          <span>{currentLang ? `${currentLang.flag} ${currentLang.name}` : 'Language'}</span>
                        </button>
                        {showLangMenu && (
                          <div className="absolute bottom-full left-0 right-0 bg-white border border-gray-100 rounded-xl shadow-lg mb-1 overflow-hidden">
                            {languages.map(lang => (
                              <button
                                key={lang.code}
                                onClick={() => {
                                  updateUserData({ preferredLanguage: lang.code });
                                  setShowLangMenu(false);
                                }}
                                className={`w-full p-3 text-left flex items-center gap-3 text-sm hover:bg-gray-50 transition-colors ${
                                  userData?.preferredLanguage === lang.code ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-700'
                                }`}
                              >
        ...
        {mobileMenuOpen && user && (
          <div className="md:hidden py-4 border-t border-gray-100 space-y-2">
            <MobileNavLink href="/" icon={<GraduationCap size={18} />} active={isActive('/')}>Levels</MobileNavLink>
            <MobileNavLink href="/videos" icon={<Video size={18} />} active={isActive('/videos')}>Videos</MobileNavLink>
            <MobileNavLink href="/tests" icon={<FileText size={18} />} active={isActive('/tests')}>Tests</MobileNavLink>
            <MobileNavLink href="/certificates" icon={<Award size={18} />} active={isActive('/certificates')}>Certificates</MobileNavLink>
            <MobileNavLink href="/chat" icon={<MessageCircle size={18} />} active={isActive('/chat')}>Chat</MobileNavLink>
          </div>
        )}
      </div>
    </nav>
